...
a monitor, our a bunch of them, don't necessarily mean that your cluster is
...
**Are you able to connect to the monitor's servers?**
  ...
  the server and, if that succeeds, try telnet'ing into the monitor's port.
...
please check your ``ceph.conf`` for an alternative path.
...
What if state is ``synchronizing``?
  ...
  smaller your monitor store is, so if you have a big enough store it may
  ...
  problem: the cluster is progressing way too fast for the synchronization
  process to keep up. This used to be a thing in early Cuttlefish, but since
  then the synchronization process was quite refactored and enhanced to avoid
  just this sort of behavior. If this happens in later versions let us know.
  And bring some logs (see `Preparing your logs`_).
...
Inject a monmap into the monitor
  ...
  feature that can cause havoc with your monitors if misused.
...
You can find us on IRC at #ceph and #ceph-devel on OFTC and on
``ceph-devel@vger.kernel.org`` and ``ceph-users@lists.ceph.com``. Make sure
you have grabbed your logs and have them ready if someone asks: the faster the
interaction and lower the latency in response, the better chances everyone's time
is optimized.
...
may want them. However, your logs may not have the necessary information.
...
Finally, you should reach out to us on the mailing lists or on IRC.
