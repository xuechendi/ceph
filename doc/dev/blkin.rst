...
Ceph.::
  ...
  export BLKIN_CFLAGS=-I/usr/local/include/
  export BLKIN_LIBS=-lblkin
...
When testing blkin, make sure lttng is ready for to collecting events.::

  lttng-sessiond -v 2>&1 > /var/log/lttng-sessiond.log &
  LD_PRELOAD=/usr/lib/x86_64-linux-gnu/liblttng-ust-fork.so ceph-osd -i ${OSD_ID} --pid-file=/var/lib/ceph/pid/ceph-osd.${OSD_ID}.pid
...
Start up an LTTng session and enable the tracepoints.::
  ...
  lttng enable-event --userspace zipkin:*
  lttng start
